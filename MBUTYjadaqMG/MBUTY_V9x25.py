...
numWires  = 120    # num of wire channels always from 0 to 31
numStrips = 12    # num of strip channels, either 0 to 31 or 0 to 63
  
positionRecon = 2

# binning position 
if positionRecon == 0:
   posBins = [120,12]     # w x s max max
elif positionRecon == 1:
   posBins = [240,24]     # w x s CoG CoG
elif positionRecon == 2:
   posBins = [120,24]     # w x s max CoG
...
if softthreshold == 0:
    ...
    [sth,softthreshold] = mbl.softThresholds(sthpath,sthfile,digitID,softthreshold,64)
...
rateHe3      = np.zeros((4,1))
rateHe3[3,0] = SingleFileDuration
...
#####################################      
    for ac in range(len(acqnum)):
        ...
        try:
                ...
                print('\033[1;31m ---> No data for digit: '+str(digitID[dd])+' ... \033[1;37m')
                flag = 2
                continue
                # print(' ---> Exiting ... \n')
                # print('------------------------------------------------------------- \n')
        ...
        if flag == 0 :
            ...
            # data_before_mapping = np.copy(data)
            
            data = mbl.mappingChToGeometry(data,MAPPING,mappath,mapfile)
            
            # data_after_mapping = np.copy(data)
...
if flag == 0 :
    print('\n \t \033[1;35mMG 2D %d events (candidates %d) in duration %d s\033[1;37m' % (NumeventNoRej,Nevents,SingleFileDurationFromFile))
...
###############################################################################
###############################################################################
#  HERE STARTS THE PART FOR THE OTHER DIGITISER 
###############################################################################
###############################################################################

# check if monitor is in the data
if MONOnOff is True: 
    if not(MONdigit in [digitOT]):
        print(' MONITOR absent in this cassette selection!')
        MONfound = False
    elif (MONdigit in [digitOT]):
        MONfound = True
...
for ac in range(len(acqnum)):
    ...
    # print('\n ---> Reading Digitizer '+str(digitID[dd])+', serial '+str(acqnum[ac]))
    print('\n \033[1;32m---> Reading Digitizer '+str(digitOT)+' for monitor, He3 tube and VMM MO, serial '+str(acqnum[ac])+'\033[1;37m')
    ...
    try:
            [data2, Ntoffi2, GTime2, _, flag2] = lof.readHDFjadaq(datapath,filenamefull,digitOT,Clockd,ordertime=True)
    except: 
            # print('\n \033[1;31m---> this looks like either a file created with the EFU file writer, change mode with EFU_JADAQ_fileType set to 1! Or JADAQ file maybe empty! \033[1;37m')
            print('\033[1;31m ---> No data for digit: '+str(digitOT)+' ... \033[1;37m')
            flag2 = 2
            continue
            # print(' ---> Exiting ... \n')
            # print('------------------------------------------------------------- \n')
    ...
    if flag2 == 0:
        ...
        if flag2 == 0:
           ...
           if np.shape(tsec)[0] > 1:
               ...
               if abs(SingleFileDurationFromFile-SingleFileDuration) > 1: #if they differ for more then 1s then warning
                   ...
                   time.sleep(2)
               Ntoffiapriori = round(SingleFileDuration/ToFduration)   
               if abs(Ntoffiapriori-Ntoffi2) >= 2: 
                   print('\n     \033[1;33mWARNING: check Num of ToFs ... found %d, expected %d \033[1;37m' % (Ntoffi2, Ntoffiapriori))
        ...
        print('\n \t \033[1;35mDuration %d s \033[1;37m' % (SingleFileDurationFromFile))
        
    ############################
    # EMMA MONITOR
        if MONfound is True and MONdigit == digitOT:
            
             selMON  = data2[:,1] == MONch
             # temp    = data2[:,[0,2]] #select only col with time stamp and charge
             MONdata = data2[selMON,:]
             MONdata[:,0] = np.around((MONdata[:,0]),decimals=6)     #  time stamp in s and round at 1us
             # data    = data[np.logical_not(selMON),:] # remove MON data from data 
             rateMON[0,0] = rateMON[0,0]+np.shape(MONdata)[0]
             rateMON[1,0] = rateMON[1,0]+SingleFileDurationFromFile
             
             # check sinslge file duration 
                 
             print('\n \t \033[1;35mMONITOR %d events \033[1;37m' % (np.shape(MONdata)[0]))
    ...
    # He3 tube
        ...
        selHe3  = data2[:,1] == He3ch
        # temp    = [:,[0,2]] #select only col with time stamp and charge
        He3data = data2[selHe3,:]
        He3data[:,0] = np.around((He3data[:,0]),decimals=6)     #  time stamp in s and round at 1us
        
        rateHe3[0,0] = rateHe3[0,0]+np.shape(He3data)[0]
        rateHe3[1,0] = rateHe3[1,0]+SingleFileDurationFromFile
            
        print('\n \t \033[1;35mHe3 tube %d events \033[1;37m' % (np.shape(He3data)[0]))
        
        if ac == 0:
          He3dataCum = He3data
        else:
          He3dataCum = np.append(He3dataCum,He3data,axis=0)
    ...
    # VMM MO
        ...
        for kk, mmo in enumerate(MOch):
            ...
            selMO  = data2[:,1] == mmo
            MOdatatemp = data2[selMO,:]
            MOdatatemp[:,0] = np.around((MOdatatemp[:,0]),decimals=6)     #  time stamp in s and round at 1us
            
            if kk == 0:
                MOdata = MOdatatemp
            else:
                MOdata = np.concatenate((MOdata,MOdatatemp), axis=0)
                
        if ac == 0:
          MOdataCum = MOdata
        else:
          MOdataCum = np.append(MOdataCum,MOdata,axis=0)       
...
# MON 
    ...
    if MONfound is True and np.shape(MONdataCum)[0]>0:
        ...
        if MONThreshold > 0:
            aboveTh = MONdataCum[:,2] >= MONThreshold
        ...
        if plotMONtofPH is True:
            ...
            pos3 = axm2.step(xener,MONPHShistCum,'m',where='mid')
...
###############################################################################
    ...
    if np.shape(He3dataCum)[0]>0:
        ...
        if He3Threshold > 0:
            aboveTh = He3dataCum[:,2] >= He3Threshold
            He3dataCum = He3dataCum[aboveTh,:]
        ...
        axHe31.set_title('He3 ToF')
        pos3 = axHe32.step(xener,He3PHShistCum,'k',where='mid')
        axHe32.set_xlabel('pulse height (a.u.)')
        axHe32.set_ylabel('counts')
        axHe32.set_title('He3 PHS')

        if calculateLambda is True :
             
            temptof  = He3dataCum[:,0]
              
            He3lamb  = tcl.ToF2lambda(He3Distance,temptof) #input m and s, output in A
                
              # append to MONdataCum col 0 ToF, col 1 PH, col 2 lambda if present 
            He3dataCum = np.append(He3dataCum,np.round(He3lamb[:,None],decimals=2),axis=1) 
...
# TO BE IMPLEMENTED 
 ...
 # ration rel eff of  MG and He3 and Monitor as a function of lambda 
...
###############################################################################
###############################################################################
#  HERE ENDS THE PART FOR THE OTHER DIGITISER 
###############################################################################
###############################################################################  
###############################################################################
###############################################################################
