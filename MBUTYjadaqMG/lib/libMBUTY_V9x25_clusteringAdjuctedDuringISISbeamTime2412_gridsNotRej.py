...
def clusterPOPH (data,Timewindow):
    ...
    ADCCH[:,4]   = data[:,1] <= 119   # wires
    ADCCH[:,5]   = np.logical_and(data[:,1] >= 121  , data[:,1] <= 132)  # grids
    ...
    for kk in range(0,NumClusters,1):
            ...
            if (ww != 0 and ss != 0 and ss <= 12 and ww <= 32 and acceptWindow): #if there is at least 1 wire and 1 strip and no ch number above 32
                ...
                if (neigw == 1 and neigs == 1):    #if they are neighbour then...
                    ...
                    POPH[kk,0]   = round((np.sum(clusterq[:,10],axis=0))/(POPH[kk,3]),2)         #position wires 0 to 119
                    POPH[kk,1]   = round((((np.sum(clusterq[:,11],axis=0))/(POPH[kk,4]))-121),2)  #position grids from 0 to 11
            ...
            elif (ss >= 1 and ww == 0 and ss <= 32 and acceptWindow):
                ...
                if (neigs == 1):    #if they are neighbour then...
                    
                    rejCounter[5] = rejCounter[5]+1  
    ...
    print("\t not rej (2D) %.1f%%, only w (1D) %.1f%%, rejected (2D or 1D) no neigh %.1f%%, rejected >32 %.1f%%, only grids %.1f%%, rejected other reasons (only strips - noise)  %.1f%% " % (rej2[0],rej2[2],rej2[1],rej2[3],rej2[5],rej2[4]));
    print("\t not rej (2D) %.1f%%, only w (1D) %.1f%%, only grids %.1f%% \n " % (rej3[0],rej3[2],rej3[5]))
...
def myHistXYZ (XX,A,YY,B,ZZ,C,coincidence=True,showStats=True):
    ...
    if showStats is True:
       ...
       print(' \t percentage in proj grids hist: %.1f%% (only s)' % (countn2))
    
    return XY, XYproj, XZ, XYprojY 
