...
pathsourceEFU         = 'efu@192.168.0.58:/home/efu/data/eszter/'
pathsourceJDQ         = ''
...
#####################################      
    for ac in range(len(acqnum)):
        ...
        if EFU_JADAQ_fileType == 0: 
            ...
            try:
                ordertime = 1
                [data, Ntoffi, GTime, _, flag] = mb.readHDFjadaq_3col(datapath,filenamefull,digitID[dd],Clockd,ordertime)
            except: 
            ...
            try:
               ordertime = 1
               [data, Ntoffi, GTime, _, flag] = mb.readHDFefu_3col(datapath,filenamefull,digitID[dd],Clockd,ordertime)
        ...
        #####################################   
        data = mb.mappingChToGeometry(data,MAPPING,mappath,mapfile)
        ...
        coincidence = 1    
        showStats   = 1
        XY, XYproj, XToF = mb.myHistXYZ(XX,POPH[:,0],YY,POPH[:,1],ToFx,POPH[:,2],coincidence,showStats)
        ...
        if calculateLambda == 1:   
           ...
           coincidence = 1    
           showStats   = 0
           __ , __ , XLam = mb.myHistXYZ(XX,POPH[:,0],YY,POPH[:,1],xlambda,POPH[:,8],coincidence,showStats)  
