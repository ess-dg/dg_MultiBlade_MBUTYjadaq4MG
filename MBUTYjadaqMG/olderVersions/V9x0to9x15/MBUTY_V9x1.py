...
###############################################################################
###############################################################################
########    V9.0 2020/02/01      francescopiscitelli    ######################
...
# correlation of PHS wires VS srtrips per digitiser (only calacualted first serial acqnum)
...
#####################################      
    for ac in range(len(acqnum)):
        ...
        if np.logical_and(plotChRaw == 1, ac == 0):
            ...
            plt.step(Xaxis,histxx,where='mid') 
...
if MONfound == 1:
    ...
    if plotMONtofPH == 1:
        ...
        pos2 = axm1.step(ToFx,MONToFhistCum,'k',where='mid')
